...
# 
# Credits to James Villeneuve for pointing out Python 3 compatibility issues
#
# ------------------------
...
def checkFile(outfile):
	...
	while (os.path.isfile(outfile) is True):
		overwrite = input('[!] Output data file "%s" already exists, overwrite? [y/n] ' % outfile)
		if overwrite in ('y', 'Y'):
			return outfile
...
def saveDAT(nodes, triangles, outfile, normals = None):
	print('[+] Saving output file: %s' % outfile)
...
def parseSTL(infile, outfile):
	...
	if param_yes is False:
		...
		print('[+] Input STL file: %s' % infile)
		print('[+] Output header file: %s' % outfile)
	stl = open(infile, 'r')
	nodes = []
	unique_nodes = []
	triangles = [[]]
	...
	check = len(nodes) % 3
	print('[+] STL file check: %s' % ('good' if check == 0 else 'bad'))
	if check != 0:
	...
	if param_verbose is True:
		for node in unique_nodes:
			...
			printVerbose('    %s, %s, %s\n' % (str(node[0]), str(node[1]), str(node[2])))
			...
			for i, node in enumerate(unique_nodes):
				...
				if [token[1], token[2], token[3]] == node:
					...
					if len(triangles[index]) < 3:
						...
						if len(triangles[index]) == 3:
							# print previous triangle created
							printVerbose('     %s, %s, %s\n' % (str(triangles[index][0]), str(triangles[index][1]), str(triangles[index][2])))
						break
	...
	# print stats
	print('[+] Vertices: %d' % len(unique_nodes))
	print('[+] Triangles: %d' % len(triangles))
	if param_normals is True:
		print('[+] Normals: %d' % len(normals))
